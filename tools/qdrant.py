...
class QdrantDB:
    ...
    def get_file_chunks(self, file_tag: str, limit: int = 10000) -> List[Dict[str, Any]]:
        """
        Return all chunks for a file, sorted by metadata.chunk_index when available.
        """
        from qdrant_client import models
        ...
        results.sort(
            key=lambda item: (
                ...
                item.get("chunk_index") if item.get("chunk_index") is not None else 10**9,
                item.get("id", ""),
            )
        )
        return results[:limit]

    def get_adjacent_chunks(self, file_tag: str, chunk_index: int, window: int = 1) -> List[Dict[str, Any]]:
        """
        Return adjacent chunks around the given chunk index in the same file.
        """
        if chunk_index is None:
            return []

        chunks = self.get_file_chunks(file_tag=file_tag)
        if not chunks:
            return []
        ...
        return [
            ...
            and abs(chunk["chunk_index"] - chunk_index) <= window
        ]
