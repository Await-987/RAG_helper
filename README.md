...
为了优化大数据量（10万+ 记录）下的响应速度，系统采用**永久缓存**策略：
...
| 组件 | 说明 | 重建触发条件 |
|-----|------|-------------|
| 数据库统计 | 文件状态、切片数量 | 入库/删除文件 |
| 词汇索引 | BM25 关键词搜索 | 数据量变化 |
...
**相关文件：**
- `tools/file_manager_ui.py` - `get_database_stats()` 数据库统计缓存
- `tools/qdrant.py` - `_maybe_build_lex_index()` 词汇索引缓存
- `docs/changes/cache_optimization_permanent.md` - 详细变更记录
